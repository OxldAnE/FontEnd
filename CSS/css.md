...
|   `div::before`   | 元素内容的前面 |
|   `div::after`    | 元素内容的末尾 |
| `p::first-letter` | 段落的首个字母 |
|  `p::first-line`  | 段落的首行文字 |
...
|            示例             |                   描述                   |
| :-------------------------: | :--------------------------------------: |
|    `ul > li:first-child`    |        列表内首个类型为表项的元素        |
|    `ul > li:last-child`     |      列表内最后一个类型为表项的元素      |
|   `ul > li:nth-child(2n)`   |   列表内偶数子节点，且类型为表项的元素   |
|  `ul > li:nth-child(2n+1)`  |   列表内奇数子节点，且类型为表项的元素   |
|  `ul > li:nth-of-type(2n)`  | 列表内类型为表项的节点中，且为偶数的元素 |
| `ul > li:nth-of-type(2n+1)` | 列表内类型为表项的节点中，且为奇数的元素 |
